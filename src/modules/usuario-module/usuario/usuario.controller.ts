...
import { routeUsuarioCreate, routeUsuarioCreateMultiple, routeUsuarioFindAll, routeUsuarioFindOne, routeUsuarioRemove, routeUsuarioUpdate } from 'src/common/routes/usuario-module/usuario.router';
...
export class UsuarioController {
  ...
  async findOne(@Param('usuario') usuario: string, @Param('password') password: string): Promise<ApiResult> {
    ...
    try {
      const result = await this.usuarioService.findOne(usuario, password);
  ...
  @Patch('/password/:usuario')
  async updatePassword(@Param('usuario') usuario: string, @Body() updateUsuarioDto: UpdateUsuarioDto): Promise<ApiResult> {
    let apiResult = { title: routeUsuarioUpdate.title, route: routeUsuarioUpdate.route, status: 'error', code: 0, message: '', boolean: false, rows: 0, data: null } as ApiResult;

    try {
      const result = await this.usuarioService.updatePassword(usuario, updateUsuarioDto);
